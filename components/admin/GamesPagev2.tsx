...
export default function AdminGamesPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const data = { ...form, platform: form.platform.split(',').map(s=>s.trim()).filter(Boolean), tags: form.tags.split(',').map(s=>s.trim()).filter(Boolean) };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="glass-card w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6">
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-4">
              ...
              ] as [string,string,boolean,string][]).map(([label, field, required, type]) => (
                <div key={field}>
                  ...
                  <input type={type} value={(form as Record<string,unknown>)[field] as string}
                    ...
                    required={required} className="input-field py-2 text-sm" />
                </div>
              ))}
              ...
              <div className="col-span-2 flex flex-wrap gap-4">
                {[['requiresServerId','Perlu Server ID'],['isActive','Aktif'],['isFeatured','Featured']].map(([field,label]) => (
                  <label key={field} className="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" checked={(form as unknown as Record<string,boolean>)[field]} onChange={e => f(field, e.target.checked)} className="w-4 h-4 accent-blue-500" />
