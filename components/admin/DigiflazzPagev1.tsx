...
export default function AdminDigiflazzPage() {
  const [activeTab, setActiveTab] = useState<'balance'|'pricelist'|'transaction'|'queue'>('balance');
            ...
            <motion.div initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }}>
              <Card style={cardStyle}>
                <div className="text-center py-4">
                  ...
                  {balance !== null
                    ? <Statistic value={balance} prefix="Rp" formatter={v => Number(v).toLocaleString('id-ID')}
                        valueStyle={{ color:'#34d399', fontWeight:900, fontSize:36 }} />
...
<Modal
  ...
  width={640}
  styles={{
    body: { background: '#13131f' },
    header: { background: '#13131f', borderBottom: '1px solid rgba(255,255,255,0.10)' },
    // Hapus 'content' - tidak didukung di versi ini
    // Gunakan classNames atau wrapperClassName untuk styling container
  }}
      ...
      {products
        ...
        .map((p, i) => (
          <div
            ...
            onClick={() => {
              setTxSku(p.buyer_sku_code);
              setPickerOpen(false);
              setPickerSearch('');
              toast.success(`SKU "${p.buyer_sku_code}" dipilih`);
            }}
          >
            <div>
